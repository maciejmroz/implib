...
float4 main(PS_INPUT p) : COLOR
{
	...
	float2 sampling_coords=float2(p.t_position.x,
							p.t_position.y+f1*sin(6.28318*(shading_space_coords.x*i1-f2)));
	return texture_sampler_bicubic(s1,sampling_coords,tx_sizes);
}
